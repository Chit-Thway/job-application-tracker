    ...
    ApplicationQuotaService quotas)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            ApplicationSortOptions.Oldest => query.OrderBy(application => application.AppliedOn)
                .ThenBy(application => application.RoleTitle),
            ApplicationSortOptions.RoleTitle => query.OrderBy(application => application.RoleTitle)
                .ThenByDescending(application => application.AppliedOn),
            ApplicationSortOptions.CompanyName => query.OrderBy(application => database.Companies
    ...
    private static Guid[] NormalizeIds(IReadOnlyCollection<Guid> applicationIds) =>
        applicationIds
            ...
            .Distinct()
            .Take(ApplicationRules.MaximumBulkSelectionCount)
            .ToArray();
