...
|---|---|
| Authentication | ASP.NET Core Identity requires verified unique email, a 12-character complex password, five-attempt lockout, and generic login/recovery responses. Open registration uses hashed six-digit email codes with ten-minute expiry, five verification attempts, a 30-second resend delay, and account/IP rate limits. Authentication cookies are HTTP-only, SameSite Lax, eight hours, and Secure-only outside Development/Testing. |
...
| Secrets | Local database and bootstrap credentials use .NET user secrets. No connection string, readable verification code, password, email token, or private record is committed. Production configuration must use host-managed settings. |
...
- Unit and integration tests cover owner isolation, authentication and recovery, open registration, email-code expiry/attempt/cooldown controls, antiforgery-backed journeys, SSRF policy and connected-IP checks, extraction limits, retention concurrency, demo privacy, security headers, no-store private caching, and minimal health output.
